...
function showTmdbConfig() {
    showPanel(6);
    $('#tmdb_key').val('');
    $('#tmdb_api_url').val('');
    $('#tmdb_image_url').val('');
    $.get('/action', { do: 'getTmdb' }, function (res) {
        try {
            const d = JSON.parse(res);
            $('#tmdb_key').val(d.key || '');
            $('#tmdb_api_url').val(d.api_url || '');
            $('#tmdb_image_url').val(d.image_url || '');
        } catch (e) {}
    });
}

function saveTmdbConfig() {
    doAction('tmdb', { key: $('#tmdb_key').val().trim(), api_url: $('#tmdb_api_url').val().trim(), image_url: $('#tmdb_image_url').val().trim() });
}
...
function showPanel(id) {
    ...
    document.getElementById('tmdbPage').classList.toggle('active', id === 6);
    document.getElementById('tab6').classList.toggle('active', id === 6);
...
$(function () {
    ...
    $('#tmdb_key, #tmdb_api_url, #tmdb_image_url').on('keydown', function (e) { if (e.key === 'Enter') { this.blur(); saveTmdbConfig(); } });
